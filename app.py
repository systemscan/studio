...
# --- CONFIGURAZIONE UFFICIALE v2.0 (RESET TOTALE) ---
...
if "reset_pacchetto" in st.session_state and st.session_state.reset_pacchetto:
    ...
    # Reimposta numeri default a 0 (Chiavi NUOVE per forzare lo zero)
    if "new_ideali" in st.session_state: st.session_state.new_ideali = 0
    if "new_proposte" in st.session_state: st.session_state.new_proposte = 0
    if "new_accettate" in st.session_state: st.session_state.new_accettate = 0
...
if scelta == "📝 NUOVA SCHEDA":
    ...
    with st.container(border=True):
        ...
        with col_ideali:
            # NUOVA KEY: new_ideali (parte da 0)
            n_ideali = st.number_input("Sedute IDEALI:", value=0, min_value=0, key="new_ideali")
        with col_proposte:
            # NUOVA KEY: new_proposte (parte da 0)
            n_proposte = st.number_input("Sedute PROPOSTE:", value=0, min_value=0, key="new_proposte")
        ...
        with col_conferma:
            # NUOVA KEY: new_accettate. Value = n_proposte (che parte da 0)
            n_accettate = st.number_input("Sedute ACCETTATE (Reali):", value=n_proposte, min_value=0, key="new_accettate")
